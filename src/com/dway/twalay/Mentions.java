 ...
 */
public final class Mentions extends BaseScreen
{
    /**
    ...
    public Mentions(final UserAccountManager m, String uname)
    {        
    	super(uname);
    	
    	//Set action menu focus
    	this.getActionMenu().b4.setFocus();
    	
    	//Initializations
    	this.m = m;
    	
    	TL = new TimelineList(UtilityClass.getMentionsTweets());
    	TL.setEmptyString("Loading Mentions...", DrawStyle.HCENTER);
    	
    	//Set callback
    	TL.setSize(UtilityClass.getMentionsTweets().size());
    	TL.setChangeListener(new FieldChangeListener(){
			public void fieldChanged(Field field, int context) {
				// TODO Auto-generated method stub
				if(field == TL){
					if(!isfirst){
						BaseTweet tweet = (BaseTweet)UtilityClass.getMentionsTweets().elementAt(TL.getSelectedIndex());
				  ...
				  UiApplication.getUiApplication().invokeLater(new Runnable() {
			            public void run() {
			            	synchronized (UiApplication.getEventLock()) {
			            		int x = 0;
			    				if(UtilityClass.getMentionsTweets().size() > 0)
			    					x = 2;
			    				else
			    					x = 1;
    ...
    public class Refresh extends Thread{
		...
		public void run(){
			try{
            	...
            	Query q = QueryComposer.count(100);    			
    			tml.startGetMentions(q, new SearchDeviceListener() {
    				
    				public void tweetFound(Tweet tweet) {
    					boolean isthere = false;
    				    for(int x = 0; x < UtilityClass.getMentionsTweets().size(); x++){
    				    	if(((BaseTweet)UtilityClass.getMentionsTweets().elementAt(x)).getTweetID().equals(tweet.getString(MetadataSet.TWEET_ID))){
    				    		isthere = true;
    				    		break;
    				    	}
    				    	
    				    	//Update tweet date and time
    				    	((BaseTweet)UtilityClass.getMentionsTweets().elementAt(x)).setTweetDate(
    				    			((BaseTweet)UtilityClass.getMentionsTweets().elementAt(x)).getRawDate());
    				    }
    				    
    				    if(!isthere){
    				    	if(todo == 1)
    				    		UtilityClass.getMentionsTweets().addElement(new BaseTweet(tweet));
    				    	else    				    
    				    		UtilityClass.getMentionsTweets().insertElementAt(new BaseTweet(tweet), 0);
    				    }
		...
		public void render(){
			if(UtilityClass.getMentionsTweets().size() > 0){
				for(int x = 0; x < UtilityClass.getMentionsTweets().size(); x++){
					BaseTweet tweet = (BaseTweet)UtilityClass.getMentionsTweets().elementAt(x);
				...
				UiApplication.getUiApplication().invokeLater(new Runnable() {
		            public void run() {
		            	synchronized (UiApplication.getEventLock()) {
		            		isfirst = true;
		            		int len = UtilityClass.getMentionsTweets().size();
		            		TL.setSize(len);
		            		TL.invalidate();
		            	}
		            }

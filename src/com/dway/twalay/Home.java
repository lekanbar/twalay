 ...
 */
public final class Home extends BaseScreen
{
    /**
    ...
    public Home(final UserAccountManager m, String uname)
    {        
    	super(uname);
    	
    	//Set action menu focus
    	this.getActionMenu().b3.setFocus();
    	
    	//Initializations
    	this.m = m;		
    	
    	TL = new TimelineList(UtilityClass.getHomeTweets());
    	TL.setEmptyString("Loading tweets...", DrawStyle.HCENTER);
    	
    	//Set callback
    	TL.setSize(UtilityClass.getHomeTweets().size());
    	TL.setChangeListener(new FieldChangeListener(){
			public void fieldChanged(Field field, int context) {
				// TODO Auto-generated method stub
				if(field == TL){
					if(!isfirst){
						BaseTweet tweet = (BaseTweet)UtilityClass.getHomeTweets().elementAt(TL.getSelectedIndex());
				  ...
				  UiApplication.getUiApplication().invokeLater(new Runnable() {
			            public void run() {
			            	synchronized (UiApplication.getEventLock()) {
			            		int x = 0;
			    				if(UtilityClass.getHomeTweets().size() > 0)
			    					x = 2;
			    				else
			    					x = 1;
    ...
    public class Refresh extends Thread{
			...
			try{
    			...
    			tml.startGetHomeTweets(q, new SearchDeviceListener() {
    				...
    				public void tweetFound(Tweet tweet) {
    				    ...
    				    for(int x = 0; x < UtilityClass.getHomeTweets().size(); x++){
    				    	if(((BaseTweet)UtilityClass.getHomeTweets().elementAt(x)).getTweetID().equals(tweet.getString(MetadataSet.TWEET_ID))){
    				    		isthere = true;
    				    		break;
    				    	}
    				    	
    				    	//Update tweet date and time
    				    	((BaseTweet)UtilityClass.getHomeTweets().elementAt(x)).setTweetDate(
    				    			((BaseTweet)UtilityClass.getHomeTweets().elementAt(x)).getRawDate());
    				    }
    				    
    				    if(!isthere){
    				    	if(todo == 1)
    				    		UtilityClass.getHomeTweets().addElement(new BaseTweet(tweet));
    				    	else    				    
    				    		UtilityClass.getHomeTweets().insertElementAt(new BaseTweet(tweet), 0);
    				    }
		...
		public void render(){
			if(UtilityClass.getHomeTweets().size() > 0){
				for(int x = 0; x < UtilityClass.getHomeTweets().size(); x++){
					BaseTweet tweet = (BaseTweet)UtilityClass.getHomeTweets().elementAt(x);
					
					boolean isthere = false;
					for(int i = 0; i < vec.size(); i++){
						BaseTweet useravatar = (BaseTweet)vec.elementAt(i);
				...
				UiApplication.getUiApplication().invokeLater(new Runnable() {
		            public void run() {
		            	synchronized (UiApplication.getEventLock()) {
		            		isfirst = true;
		            		int len = UtilityClass.getHomeTweets().size();
		            		TL.setSize(len);
		            		TL.invalidate();
		            	}
		            }
			  ...
			  try {
				  for(int i = 0; i < vec.size(); i++){
					  ...
					  if(!useravatar.getIsDownloaded()){
						  ...
						  ((BaseTweet)vec.elementAt(i)).setAuthorImage(img);
						  ((BaseTweet)vec.elementAt(i)).setIsDownloaded(true);

 ...
 */
public final class Search extends BaseScreen
{
    /**
    ...
    public Search(final UserAccountManager m, String query)
    {        
    	super("Search");
    	
    	//Set action menu focus
    	this.getActionMenu().b6.setFocus();
		...
		ButtonField bp = new ButtonField("Search");
		bp.setChangeListener(new FieldChangeListener(){
			public void fieldChanged(Field field, int context) {
				...
				Search.this.query = txtTweetBox.getText();
				new Refresh(x).start();
			}
    	...
    	if(query.equals(""))
    		TL.setEmptyString("No Result", DrawStyle.HCENTER);
    	else
    		TL.setEmptyString("Serching tweets...", DrawStyle.HCENTER);
    ...
    public class Refresh extends Thread{
		...
		public void run(){
			try{
            	...
            	q = QueryComposer.append(q, QueryComposer.containHashtag(query));
            	q = QueryComposer.append(q, QueryComposer.containAny(query));
